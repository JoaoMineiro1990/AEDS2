...
public class TP02Q018
{
    ...
    public static void QuickSort (int esq, int dir, int parcial, Jogador[] jogadores)
    {
        int i = esq;
        int j = dir;
        String comparar;
        comparar = jogadores[(esq+dir)/2].getEstadoNascimento();

        while (i <= j) 
        {
            while (jogadores[i].getEstadoNascimento().compareTo(comparar) < 0)
            {
                i++;
                comp++;
            }
            while (jogadores[j].getEstadoNascimento().compareTo(comparar) > 0)
            {
                j--;
                comp++;
            }
        ...
        {
            comp++; 
            QuickSort(esq , j , parcial , jogadores);
        }
        if (i < parcial && i < dir)
        {
            comp+=2;
            QuickSort(i , dir , parcial , jogadores);
        } 

    }
    public static void selec (int parcial, int tamanho, Jogador[] jogadores)
    {
        for (int i = 0; i < parcial; i++) 
        {
            ...
            {
                comp+=2;
                if ((jogadores[menor].getEstadoNascimento()).compareTo(jogadores[j].getEstadoNascimento()) == 0 && jogadores[menor].getNome().compareTo(jogadores[j].getNome()) > 0)
                {
                    menor = j;
                }
            }
    ...
    public static void main (String [] args) throws IOException
    {
        ...
        String nomeDoArquivo = "816594_parcial_QuickSort.txt";
        ...
        {        
            for(int u = 0; u < k ; u++)
            {
                System.out.println(jogadores[u].Imprimir());
            }
        }
        else
        {
