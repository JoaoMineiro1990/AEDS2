...
public class TP02Q011
{
    ...
    public static void sort(int tamanho, Jogador[] jogadores) 
    {
        // jogadores para contar o número de ocorrências de cada elemento

        int[] count = new int[getMaior(tamanho,jogadores) + 1];
        Jogador[] ordenado = new Jogador[tamanho];
    
        // Inicializar cada posição do jogadores de contagem
        for (int i = 0; i < count.length; i++) 
        {
            count[i] = 0;
        }
    
        // Primeiro loop: Contando as ocorrências de alturas
        for (int i = 0; i < tamanho; i++) 
        {
            int altura = jogadores[i].getAltura();
            count[altura]++;
        }
    
        // Segundo loop: Modificando o jogadores de contagem para conter números acumulados
        for (int i = 1; i < count.length; i++) 
        {
            count[i] += count[i - 1];
        }
    
        // Terceiro loop: Ordenando o jogadores com base nas alturas e no jogadores de contagem
        for (int i = tamanho - 1; i >= 0; i--) 
        {
            // pegando altura
            int altura = jogadores[i].getAltura();
            ordenado[count[altura] - 1] = jogadores[i];
            count[altura]--;
        }
        

        // Copiando o jogadores ordenado de volta para o jogadores original
        for (int i = 0; i < tamanho; i++) 
        {
      ...
      public static int getMaior(int tamanho, Jogador[] jogadores) 
    {
        int maior = jogadores[0].getAltura();
            ...
            {
                comp++;
                if(maior < jogadores[i].getAltura())
                {
    ...
    public static void main (String [] args) throws IOException
    {
        ...
        String nomeDoArquivo = "816594_countingsort.txt";
